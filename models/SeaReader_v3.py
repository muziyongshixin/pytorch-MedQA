...
class SeaReader_v3(torch.nn.Module):
    ...
    def forward(self, contents, question_ans, logics, contents_char=None, question_ans_char=None):
        ...
        viewed_matching_feature_col=torch.stack(matching_feature_col,dim=1).view(batch_size*self.use_content_nums,max_question_len,2) # size 16*n,100,2
        viewed_matching_feature_row=torch.stack(matching_feature_row,dim=1).view(batch_size*self.use_content_nums,max_content_len,2) # size 16*n,200,2
